...
export default function HeaderMessage() {
  ...
  const checkIsServiceAccountInitialized = () => {
    isAccountInitializedTx(publicConfig.flowAddress).then(data => {
      setIsServiceAccountInitialized(data.CardItems && data.CardItemsMarket)
    })
  }
      ...
      <HeaderContainer>
        {isServiceAccountLoggedIn ? (
          <>
            ...
            the Service Account to mint Card Items.
          </>
        ) : (
          <>
            {currentUser ? (
  ...
  return (
    <HeaderContainer>
      <span className="mr-3 text-sm">💻</span>Card Items is a demo application
